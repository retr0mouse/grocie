...
export default function SmallProduct(props: Props) {
    ...
    return (
        <div className="pr-5 pl-5 bg-white text-center flex-[1_0_15%] w-60 h-[400px] mt-16 rounded-2xl flex-col flex">
            ...
            <Link href={{ pathname: `/product/${props.name}`, query: { product: JSON.stringify(thisProduct), count: JSON.stringify(counter) }}} as={`/product/${props.name}`} className="self-center mt-4 place-content-center place-items-center flex flex-col transition ease-in-out delay-50  hover:text-orange-700 duration-200">
